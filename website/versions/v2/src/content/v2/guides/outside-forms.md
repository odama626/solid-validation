...
`use:validate` works on any element. Give it a `data-name` so its errors have a key, and call `submit()` yourself instead of relying on a form:
...
Registered elements all live in the same form, whether they are inputs or not. A `<form use:formSubmit>` containing a `div use:validate` validates both on submit, and a single `errors` store covers them.
